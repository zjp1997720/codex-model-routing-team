...
- 可以作为 Deep Research 等上游 Skill 的 Thread Orchestrator，保留上游流程、阶段门和产出标准。
...
上游 Skill 已经完成任务拆分时，本 Skill 直接接受其 Scale 和阶段顺序，只负责模型、Thread 生命周期与安全额度。声明工作区输出路径的任务始终绑定匹配项目，不会因为主题属于“通用调研”而进入 projectless。

Deep Research 默认使用 `2-4 researcher + 1 verifier + 1 reviewer + 2 retry reserve`，确保累计 8 个任务额度仍能覆盖引用核查、对抗审查和失败恢复。
