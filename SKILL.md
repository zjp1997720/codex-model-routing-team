...
2. 读取 [路由策略](references/routing-policy.md)，判断并行收益是否高于协调成本。简单任务直接执行，不创建后台任务。
3. 选择模式：默认轻量路由；满足长期、正式交付、可恢复或高风险条件时，读取 [耐久模式](references/durable-mode.md)。
4. 先显示一条简短派遣通知：任务数、每个任务的模型、推理强度和职责。
5. 读取 [任务包模板](references/task-packet.md)，为每个 Worker 写完整提示词。提示词必须包含“禁止创建任何后台任务或子 Agent”。
6. 读取 [任务生命周期](references/thread-lifecycle.md)。用 `codex_app__list_projects` 定位项目；工作区写入任务使用匹配项目，通用调研使用 projectless。用 `codex_app__create_thread` 显式传入 `model` 与 `thinking`。
...
8. 健康探针通过后，每波最多再创建 3 个；运行并发最多 6 个，单个根任务累计最多创建 8 个。按文件、模块、章节或主题分配互斥所有权；同一文件同一时刻只允许一个写入者。
...
10. 主 Agent 亲自核对事实、运行验证、处理冲突并整合最终交付。只对已实体化、状态为 completed/idle、且结果已采纳的轻量任务调用 `codex_app__set_thread_archived`；逐个归档并等待每次确认。失败、争议或待审任务保留。
11. 最终汇报任务数、模型分布、升级/重试、采纳结果、归档情况和未解决风险。
...
交付必须完整、自洽、经过主 Agent 验证，并包含可审计的模型路由摘要。触发、并发、归档和失败边界见 [验证案例](references/validation-cases.md)。
